...
export default {
  ...
  methods: {
    async createTask() {
      ...
      if (!this.title || !this.body || !this.dueDate) {
        this.error = "Title, body or due date empty";
        return;
      }
      await axios
        .post(
    ...
    async getTasks() {
      ...
      this.loading = true;
      await axios
        .get(BASE_URL + "/tasks", { headers: { Authorization: token } })
        .then((res) => {
          console.log(JSON.stringify(res.data, null, 2));
          this.tasks = res.data;
    ...
    async submitFile(taskItem) {
      ...
      const config = {
        headers: {
          "Content-Type": this.attachment.type,
          "Authorization": token,
          "X-File-Name": this.attachment.name,
        },
      };
      ...
      const res = await instance
        .put(BASE_URL + `/upload/${taskId}`, this.attachment, config)
        .catch((err) => {
          console.log(err);
          this.error = `Error uploading files: ${err.message}`;
        });
